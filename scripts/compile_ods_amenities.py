...
Inputs: 
    - Education dataset
    - NoRMS stations (centroids)
    - Hospital locations
    - Supermarket locations
    - Postcode to Lat, Long lookup
    - GP Surgery Postcode data
...
# Get path for data dir (should be within \scripts now)
os.chdir('..')
...
print('\nLSOAs loaded')
#### Load Education Data #### 
education_path = os.path.join(data_dir, 'EduBase Extract - 2016-0005414.csv')
...
# GP surgeries
GP_path = os.path.join(data_dir, 'epraccur.csv')
...
employment_fn = '501 Employment centres.csv'

# Load employment centres data
lsoa_emp_amenities = pd.read_csv(os.path.join(data_dir, 
                                                employment_fn))
towns_fn = '508 Town centres.csv'
# Load Town centre access data
lsoa_town_amenities = pd.read_csv(os.path.join(data_dir,
                                                 towns_fn))
...
# Join on Town access & Employment centres data
LSOA_amenities = LSOA_amenities.merge(how='left',
                                      right=lsoa_town_amenities,
                                      left_on='LSOA11CD', 
                                      right_on='LSOA_code')
# Formatting
LSOA_amenities.drop(columns='LSOA_code', inplace=True)
# Add employment centres
LSOA_amenities = LSOA_amenities.merge(how='left',
                                      right=lsoa_emp_amenities,
                                      left_on='LSOA11CD',
                                      right_on='LSOA_code')
# Formatting
LSOA_amenities.drop(columns='LSOA_code', inplace=True)

# Add amenity totals count per LSOA for OTP filtering
...
# Save the data
LSOA_amenities.to_csv(os.path.join(data_dir, 'LSOA_amenities.csv'),
                      index=True)
